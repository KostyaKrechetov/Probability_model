...
for index_, well_num in enumerate(sw_excel.sheet_names[:-1]):
    try:
        ...
        time = pd.to_datetime(sw_hist_sheet.iloc[[index for index, row in
                                                  sw_hist_sheet.iterrows() if row.iat[0] >= datetime(2019, 11, 1)], 0])
        k = len(time)
        ...
        xgb_model = shared(xgb_hist_sheet.iloc[[index for index, row in xgb_hist_sheet.iterrows() if
                                                row.iat[7] in list(time)], 8].values)
        eln_model = shared(eln_hist_sheet.iloc[[index for index, row in eln_hist_sheet.iterrows() if
                                                row.iat[7] in list(time)], 8].values)
        ...
        with pm.Model() as model_g:
            ...
            trace_g = pm.sample(sample, chains=1, tune=tune, target_accept=.95, cores=1)
        ...
        time = pd.to_datetime(
            sw_sheet.iloc[[index for index, row in sw_sheet.iterrows() if row.iat[0] >= datetime(2020, 2, 1)], 0])
        k = len(time)
        ...
        delta = sw_sheet.iloc[k - 1, 1] - (
                ...
                + w_xgb_m * xgb_sheet.iloc[
                    [index for index, row in xgb_sheet.iterrows() if row.iat[2] == sw_sheet.iloc[k - 1, 0]], 3].values[
                    0]
                + w_eln_m * eln_sheet.iloc[
                    [index for index, row in eln_sheet.iterrows() if row.iat[2] == sw_sheet.iloc[k - 1, 0]], 3].values[
                    0])

        # In[] Генерация сэмплов прогнозов из построенной модели. Нужно для того, чтобы было много сэмплов для
        # построения доверительного интервала
        ppc2 = pm.sample_posterior_predictive(trace_g, samples=50, model=model_g)
