...
# In[] Загрузка данных
file_name = 'Несколько моделей для ансамбля.xlsx'
sw_excel = pd.ExcelFile(file_name)

file_name = 'Ошибки моделей\\XGBoost.xlsx'
xgb_excel = pd.ExcelFile(file_name)

file_name = 'Ошибки моделей\\Elastic_net.xlsx'
...
# for index_, well_num in enumerate(sw_excel.sheet_names[-7:-1]):
    try:
        ...
        time = pd.to_datetime(sw_sheet.iloc[[index for index,row in sw_sheet.iterrows() if row.iat[0] < datetime(2020, 2, 1)
                                             and row.iat[0] in list(eln_sheet.iloc[:, 2])], 0])
        k = len(time)
        n = len(sw_sheet.iloc[[index for index,row in sw_sheet.iterrows() if row.iat[0] < datetime(2020, 2, 1)], 0])
        k = n - k
        time_zero = time.values[-1]
        time_deltas = shared(np.array([abs((elem - time_zero).days) for elem in time]))

        Q_real=shared(sw_sheet.iloc[k:n,1].values)
        Q_model1=shared(sw_sheet.iloc[k:n,2].values)
        Q_model2=shared(sw_sheet.iloc[k:n,3].values)
        Q_model3=shared(sw_sheet.iloc[k:n,4].values)
        Q_model4=shared(sw_sheet.iloc[k:n,5].values)
        # Q_model5=shared(sw_sheet.iloc[k:n,6].values)
        xgb_model = shared(xgb_sheet.iloc[[index for index,row in xgb_sheet.iterrows() if row.iat[2] in list(time)], 3].values)
        eln_model = shared(eln_sheet.iloc[[index for index,row in eln_sheet.iterrows() if row.iat[2] in list(time)], 3].values)
        ...
        with pm.Model() as model_g:
            ...
            trace_g = pm.sample(2000, tune=1000, target_accept=.95, cores=-1)
        ...
        plt.savefig(str(well_num) + ' - обучение.png', dpi=200, bbox_inches='tight')
        plt.tight_layout()
        plt.show()

        # In[]
        # Оценка сдвига последней точки факта от прогноза. Это сдвиг будет использован для сдвига будущего прогноза
        delta = sw_sheet.iloc[n - 1 ,1] - (
                w0_m + w1_m * Q_model1.get_value()[-1] + w2_m * Q_model2.get_value()[-1] + w3_m *
                Q_model3.get_value()[-1]
                + w4_m * Q_model4.get_value()[-1]
        # + w5_m * Q_model5.get_value()[-1]
        + w_xgb_m * xgb_model.get_value()[-1]
        + w_eln_m * eln_model.get_value()[-1])

        # In[] Построение модели на периоде прогноза
        time = pd.to_datetime(sw_sheet.iloc[n:, 0])
        # Переопределение входных параметров модели
        Q_model1.set_value(sw_sheet.iloc[n:, 2])
        Q_model2.set_value(sw_sheet.iloc[n:, 3])
        Q_model3.set_value(sw_sheet.iloc[n:, 4])
        Q_model4.set_value(sw_sheet.iloc[n:, 5])
        # Q_model5.set_value(sw_sheet.iloc[n:, 6])
        xgb_model.set_value(
        ...
        plt.plot(time, sw_sheet.iloc[n:, 1].values, 'C0.')
        plt.plot(time, Q_model1.get_value(), 'g--', label='Q_модель1')
        plt.plot(time, Q_model2.get_value(), 'b--', label='Q_модель2')
        plt.plot(time, Q_model3.get_value(), 'y--', label='Q_модель3')
        plt.plot(time, Q_model4.get_value(), 'm--', label='Q_модель4')
        ...
        plt.savefig(str(well_num) + ' - прогноз.png', dpi=200, bbox_inches='tight')
        plt.show()
...
wb.save('Ошибки моделей\\Ансамбль прогноз со статистич моделями оставшиеся.xlsx')
